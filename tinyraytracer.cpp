...
// Thanks to this : https://github.com/ssloy/tinyraytracer/wiki/Part-1:-understandable-raytracing
struct Light
{
    Vec3f position;
    float intensity;
...
struct Material
{
    ...
    Material() : refractive_index(1), albedo(1.0, 0.0, 0.0, 0.0), diffuse_color(), specular_exponent() {}
...
{
    float cosi = -std::max(-1.f, std::min(1.0f, I * N));
...
bool scene_intersect(const Vec3f& orig, const Vec3f& dir, const std::vector<Sphere>& spheres, Vec3f& hit, Vec3f& N, Material& material)
{
    ...
    if (fabs(dir.y) > 0.001)
    {
        ...
        if (d > 0 && fabs(pt.x) < 10 && pt.z < -10 && pt.z>-30 && d < sphere_dist)
        {
            ...
            hit = pt;
            N = Vec3f(0, 1, 0);
            material.diffuse_color = (int(0.5 * hit.x + 1000) + int(0.5 * hit.z)) & 1 ? Vec3f(1) : Vec3f(0.3, 0.3, 0.3);
...
Vec3f cast_ray(const Vec3f& orig, const Vec3f& dir, const std::vector<Sphere> spheres, const std::vector<Light>& lights, size_t depth = 0)
{
    ...
    for (size_t i = 0; i < lights.size(); i++)
    {
        ...
        if (scene_intersect(shadow_orig, light_dir, spheres, shadow_pt, shadow_N, tempMaterial) && (shadow_pt - shadow_orig).norm() < light_distance)
            continue;
...
void render(const std::vector<Sphere> spheres, const std::vector<Light>& lights) {
    ...
    for (size_t j = 0; j < height; j++) { //actual rendering loop
        for (size_t i = 0; i < width; i++) {
            ...
            framebuffer[i + j * width] = cast_ray(Vec3f(0), Vec3f(dir_x, dir_y, dir_z).normalize(), spheres, lights);
        }
    }
